  ...
  (fn [beat]
    (when (state-maps/active-mode? state mode)
      (let [current-x (int (mod (dec beat) phrase-size))
            idxs (take grid/grid-width (drop current-x (cycle (range 0 phrase-size))))
        ...
        (when-not (state-maps/session-mode? state)
          (doall
           (map-indexed
            (fn [x absolute-x]
              (when (< absolute-x (state-maps/x-max state))
                (doseq [y (range 0 grid/grid-height)]
                  (let [y-page (state-maps/grid-y state)
                    ...
                    (device/render-cell lp (state-maps/absolute-cell state absolute-y absolute-x) y x intensity color)))))
            idxs)))))))
